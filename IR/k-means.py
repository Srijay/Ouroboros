...
use_registered_points = False

if(use_registered_points):
    with open('./C/registered_dict_c.pkl', 'rb') as file:
        registered_coordinates_dict = pickle.load(file)
...
def create_image(coordinates, radius = 5, use_registered=False):
    if(use_registered):
        coordinates = [registered_coordinates_dict[tuple(tuple_)] for tuple_ in coordinates]
...
result_image.save('k_means_c_2.png')

if(use_registered_points):
    result_image = create_image(spectral_coordinates_list, use_registered=use_registered_points)
    result_image.save('k_means_c_registered.png')